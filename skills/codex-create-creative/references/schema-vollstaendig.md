# Vollständige Offenlegung: `image_gen__imagegen`

Stand: 2026-07-14

## 1. Exponierte Tool-Signatur

### Exakte, vollständig exponierte Deklaration

Die Session exponiert die folgende Deklaration wörtlich:
...
### JSON-Schema-Darstellung der exponierten Signatur
...
| Parameter | Typ | Pflicht/optional | Default | Enum-Werte | Originale Parameterbeschreibung |
|---|---|---|---|---|---|
| `prompt` | `string` | Pflicht | kein Default exponiert | keine Enum-Werte exponiert | Keine individuelle `description` in der Tool-Signatur exponiert. |
| `referenced_image_paths` | `Array<string> \| null` | optional | kein Default exponiert | keine Enum-Werte exponiert | Keine individuelle `description` in der Tool-Signatur exponiert. |
| `num_last_images_to_include` | `number \| null` | optional | kein Default exponiert | keine Enum-Werte exponiert | Keine individuelle `description` in der Tool-Signatur exponiert. |

Die exponierte Signatur enthält keine individuellen JSON-Schema-`description`-Felder, keine `default`-Werte, keine `enum`-Werte, kein `additionalProperties`-Feld und keine Zahlen- oder Arraygrenzen. Solche Felder werden deshalb nicht erfunden.

### Vollständige exponierte Tool-Beschreibung
...
## 2. Vollständige lokale Skill-Dokumentation

Dateipfad: `/Users/giulianocosta/.codex/skills/.system/imagegen/SKILL.md`

Nachfolgend der vollständige, unveränderte Inhalt dieser Datei:
...
## 3. Empirische Fakten aus dieser Session

In der sichtbaren Session wurden tatsächlich **zwei**, nicht drei, Aufrufe von `image_gen__imagegen` ausgeführt. Eine dritte Generierung ist im verfügbaren Sitzungsverlauf nicht vorhanden und wird daher nicht erfunden.

| Aufruf | Eingabe | Ergebnis | Ungefähre Tool-Dauer |
|---|---|---|---|
| Test 1 | nur `prompt`; beide Bildparameter weggelassen | `test1.png`, PNG, 1254 × 1254 px, 8-bit RGB, ohne Alpha | ca. 18,4 Sekunden |
| Test 2 | `prompt` plus `referenced_image_paths` mit einem lokalen PNG-Pfad | `test2_fidelity.png`, PNG, 1254 × 1254 px, 8-bit RGB, ohne Alpha | ca. 83,9 Sekunden |

- `referenced_image_paths` akzeptierte ein Array mit einem absoluten lokalen PNG-Dateipfad.
- Vor dem Edit-Aufruf wurde die lokale Datei entsprechend der Tool-Anweisung mit `view_image` geladen.
- `num_last_images_to_include` wurde in dieser Session nicht empirisch getestet.
- Das gleichzeitige Setzen beider Bildparameter wurde bewusst nicht getestet, weil die Tool-Anweisung dies ausdrücklich verbietet. Das konkrete Laufzeit-Fehlerverhalten ist unbekannt.
- Beide Ausgaben wurden zunächst unter `$CODEX_HOME/generated_images/...` abgelegt und anschließend ins Arbeitsverzeichnis kopiert.
- Die native Signatur besitzt keinen Zielpfadparameter.
- Die 1:1-Anforderung wurde nur im Prompt formuliert.

## 4. Bekannte Grenzen

### Referenzbilder

- `num_last_images_to_include`: dokumentiertes Maximum **5** kürzlich sichtbare Gesprächsbilder.
- `referenced_image_paths`: keine maximale Arraylänge angegeben; Maximum unbekannt.
- Beide Mechanismen gleichzeitig: laut Tool-Anweisung **verboten**.
- Wenn nicht alle Zielbilder mit einem Mechanismus eingebunden werden können, sollen fehlende Bilder erneut angehängt werden.

### Eingabeformate

- Empirisch bestätigt: lokales **PNG**.
- Weitere Formate sind nicht aufgelistet und daher unbekannt.
- Keine exponierten Dateiendungs-, MIME-, Datei- oder Pixelgrenzen.

### Native Steuerparameter

- Größe: **NEIN**
- Qualität: **NEIN**
- Seitenverhältnis: **NEIN**
- Ausgabeformat: **NEIN**
- Modell: **NEIN**
- Seed: **NEIN**
- Anzahl/`n`: **NEIN**
- Maske: **NEIN**
- `input_fidelity`: **NEIN**
- Hintergrund/Transparenz: **NEIN**
- Zielpfad: **NEIN**

Diese Aussagen gelten nur für `image_gen__imagegen`; die Skill-Datei beschreibt separat einen CLI-Fallback.

### Transparenz und Alpha

- Kein nativer Alpha- oder Transparenzparameter.
- Beide empirischen Ausgaben waren RGB-PNGs ohne Alpha.
- Der eingebaute Workflow erzeugt Transparenz laut Skill-Dokumentation über Chroma-Key plus lokale Nachbearbeitung.
- Echte modellnative Transparenz wird dort als separater CLI-Fallback mit `gpt-image-1.5` beschrieben.

### Batch-Fähigkeit

- Kein `n`-, Batch- oder Prompt-Array-Parameter im nativen Tool.
- Mehrere native Ergebnisse erfordern laut Skill-Dokumentation mehrere einzelne Aufrufe.
- `generate-batch` gehört ausschließlich zum separaten CLI-Fallback.

### Modellidentität

- Der konkrete serverseitige Modellname wird nicht exponiert.
- `gpt-image-2` wird in der Skill-Datei als Standard des CLI-Fallbacks dokumentiert; dies beweist nicht die Modellidentität des nativen Tools.

## 5. Codex-Konfiguration und CLI-Flags

Für das native Tool `image_gen__imagegen`: **keine bekannt**.

Die Suche nach `imagegen`, `image_gen`, `image generation` und `gpt-image` in der vorhandenen `/Users/giulianocosta/.codex/config.toml`, weiteren `.toml`-Dateien auf dieser Ebene sowie den projektlokalen Verzeichnissen `.codex` und `.agents` ergab keine Treffer.

Die in der zitierten Skill-Datei genannten CLI-Optionen und Subcommands gehören zum separaten Fallback-Skript `scripts/image_gen.py`; sie aktivieren oder konfigurieren die native Tool-Signatur nicht. Es wurde kein CLI-Fallback ausgeführt, kein API-Key verwendet und kein weiteres Bild generiert.
